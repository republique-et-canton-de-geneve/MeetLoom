...
- **Every table belongs to the snapshot.** A new table goes into `SNAPSHOT_TABLES` (durable data, parents before children) or `TRANSIENT_TABLES` (tied to one environment: sign-ins, one-time links, queues) in `server/snapshot.ts`; `tests/snapshot.test.ts` fails otherwise. Backups and export/import depend on it.
