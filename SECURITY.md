...
- **Visitor link addresses:** stored as a SHA-256 hash for lookup and, so owners can copy them again, sealed with AES-256-GCM under a random per-installation key (`installation_keys`). That key is never exported: exports, downloaded backups and imports into another environment carry no readable address. Someone reading the database directly can open them, as they can already read every session.
- **Server logs** (`server_logs`, shown to administrators) hold routes, error names and codes, never request bodies, query strings, passwords, tokens or answers. User reports hold what their author wrote; forwarding one to GitHub is a draft in the administrator's browser, without the author.
- **Outgoing TLS:** certificates of the LLM, OIDC provider and SMTP relay are verified; an internal authority is added with `NODE_EXTRA_CA_CERTS`. The one opt-in exception, `LLM_ALLOW_SELF_SIGNED=true`, skips verification for LLM calls only and is shown in the installation settings.
