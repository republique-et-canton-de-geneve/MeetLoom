...
Next steps for the user: configure Docker Hub secrets/variable, protect `main`, merge PR #1, publish `v0.1.0`, then run the OpenShift installer in development. The user deploys from Windows (PowerShell 7) and the cluster pulls through the state Nexus mirror: `-Image docker-all.devops.etat-ge.ch/jpfroud/meetloom:<version>`, with `-DatabaseImage` if `quay.io` is unreachable (see deployment.md).
