...
| AI           | Server service compatible with `/chat/completions`, configurable endpoint/model/key, optional feature | Configurable internal Qwen provider, no direct browser-to-LLM call                                                                              |
...
MeetLoom follows this model with `QWEN_BASE_URL`, `QWEN_MODEL`, an optional API-key secret, timeout, and response-size limits. The operator configures the actual model and its identifier. Supply an internal CA through Node's TLS configuration; do not default to RetroGemini's option for disabling certificate verification.
